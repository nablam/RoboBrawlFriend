...
public class MyFpsMon : MonoBehaviour
{
    int tick = 0;
    float elapsed = 0;
    float fps = 0;
    ...
    {
        EventsManagerLib.On_NDI_startedStreaming += GM_StreamStartedAck;
    }
    private void OnDisable()
    {
        EventsManagerLib.On_NDI_startedStreaming -= GM_StreamStartedAck;
    }
    void GM_StreamStartedAck(int argw, int argh)
    {
         Add("width", argw.ToString());
         Add("height", argh.ToString());
    }
    // Use this for initialization
    void Start()
    {
        console_labelStyle = new GUIStyle();
        console_labelStyle.fontSize = 20;
