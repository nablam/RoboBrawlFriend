...
sealed class FormatConverter : IDisposable
{
    ...
    void CheckDimensions(int width, int height)
    {
            ...
            if (((width & 0xf) != 0) || ((height & 0x7) != 0)) {

                if ((width & 0xf) != 0)
                {
                    defaultw = width;
                    WarnWrongSize($"Width ({defaultw}) must be a multiple of 16.");
                }

                if ((height & 0x7) != 0)
                {
                    defaulth = height;
                    WarnWrongSize($"Height ({defaulth}) must be a multiple of 8.");
                }

                EventsManagerLib.CALL_NDIWarningT_evnt(defaultw, defaulth);
